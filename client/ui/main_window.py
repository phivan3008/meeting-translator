...
unit-tested there). Excluded from the CPU test suite by design (no test
file imports it) regardless of whether PySide6 happens to be installed in
a given environment, and only truly exercised -- real widgets, real audio
hardware, a real display -- manually on Windows; see
IMPLEMENTATION_STATUS.md for the `WINDOWS-UI-*` manual verification
history. PySide6/PyAudioWPatch were installed into the Phase 11 packaging
session specifically to verify the PyInstaller build (see
``docs/DEPLOYMENT.md``'s packaging section); that makes this file
importable and mypy-checkable against real Qt stubs in that session, but
is not itself a hardware/behavioral verification of the running UI --
treat this file as behaviorally unverified outside an actual
``WINDOWS-UI-*``-style manual pass.
"""
...
_LOG = logging.getLogger("client.ui.main_window")
...
class MainWindow(QMainWindow):  # type: ignore[misc]  # PySide6 has no stubs installed here
    ...
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle(f"Meeting Translator v{__version__}")
        self.resize(900, 600)
    ...
    def _start_session(self) -> None:
        ...
        session = SessionController(
            ...
            on_fatal_error=self._bridge.fatal_error.emit,
        )
        ...
        session.start()
        # The capture-drain timer is intentionally NOT started here -- see
        # _drain_capture's docstring. It starts on the first CONNECTED
        # state instead (_on_state_changed).
    ...
    def _start_capture(self, streams: list[StreamConfig]) -> None:
        ...
        for stream_config in streams:
            ...
            _LOG.info(
                "capture started: source=%s device_index=%s stream_number=%s "
                "sample_rate=%s channels=%s",
                source.value,
                device_index,
                stream_config.stream_number,
                backend_stream.audio_format.sample_rate,
                backend_stream.audio_format.channels,
            )
        if not self._captures:
            _LOG.warning(
                "no capture stream started -- every enabled source lacked a "
                "device_index (check device selection before Connect)"
            )
    ...
    def _drain_capture(self) -> None:
        """Send captured frames -- only actually runs once connected.

        Deliberately not started until the first ``CONNECTED`` state (see
        ``_on_state_changed``), not immediately after ``session.start()``:
        starting it earlier let ``send_audio()`` run (and queue packets)
        before the WebSocket handshake completed, which meant those
        already-queued packets got sent once via ``AudioSender``'s
        reconnect-oriented resend-pending step (intended for real
        reconnects, not first connects) and then sent *again* once the
        normal outgoing pump started -- a real, observed bug
        (`WINDOWS-UI-005`'s server log showed spurious "lost packets
        (jitter overflow)" warnings on an otherwise healthy first
        connection, consistent with this extra burst of duplicate traffic
        tripping the jitter buffer's bounded reorder window). Capture
        itself (into ``CaptureContext``'s bounded queue) still starts
        immediately in ``_start_capture`` -- only sending is deferred --
        so at most a short, harmless amount of audio is buffered (and
        drop-oldest if the connect takes unusually long) rather than lost
        track of.
        """
        ...
        try:
            ...
            # The background session thread died between ticks (e.g. a
            # connection failure that exhausted this run) -- stop cleanly
            # instead of raising the same error on every 20ms tick.
            _LOG.warning("session died mid-capture; stopping")
            self._stop_session()
    ...
    def _on_state_changed(self, state: object) -> None:
        ...
        if state is ConnectionState.CONNECTED and self._session is not None:
            # Safe to call every time we (re)connect: QTimer.start() on an
            # already-running timer just keeps it running. See
            # _drain_capture's docstring for why this waits for CONNECTED
            # rather than starting right after session.start().
            self._capture_timer.start()

    def _on_fatal_error(self, error: object) -> None:
        if not isinstance(error, BaseException):
            return
        self._stop_session()
        QMessageBox.critical(self, "Connection failed", str(error) or type(error).__name__)
...
def run() -> int:
    ...
    from shared.logging import configure_logging  # noqa: PLC0415
